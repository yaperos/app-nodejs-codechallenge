...
import { GetTransactionDto } from "../../infrastructure/dto/get-transaction.dto";
  ...
  implements IQueryHandler<GetTransactionQuery>
{
  ...
  async execute(
    ...
    const transaction = await this.transactionMongooseRepo.findOne(
      getTransactionQuery.id,
    );
    if (!transaction)
      throw new NotFoundException(
        `No se encontró transacción con ID ${getTransactionQuery.id}`,
      );
    return this.transformTransactionToOutput(transaction);
  }

  private transformTransactionToOutput(
