...
export class UserUseCases {
  ...
  async createUser(request: any): Promise<any> {
    ...
    try {
      const userExists = await this.userRepository.userExists(request.payload.email);
      if (userExists) return failure(Result.fail<void>('User already exists'));
      const userID = UserID.create().getValue();
      const userPassword = UserPassword.create(request.payload.password).getValue();
      const userPayload = {
        ...request.payload,
        id: userID.id,
        password: await userPassword.getHashedPassword(),
      };
      const createdUser = await this.userRepository.createUser(userPayload, true);
