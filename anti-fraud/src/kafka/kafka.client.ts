...
import { TransactionValidation } from '../domain/transaction-validation';
    ...
    await this.consumer.run({
      eachMessage: async (payload: EachMessagePayload) => {
        ...
        if (value !== null && value !== undefined) {
          console.log({ value: value.toString(), topic: payload.topic });

          const response = TransactionValidation.validateTransaction(value.toString());
