...
export class TransactionKafkaService {
    constructor(private readonly kafkaService: KafkaService) {
        this.kafkaService.subscribeToTopic(AppService.kafka_topic, this.receiveResult.bind(this));
    }

    async sentStatus(transactionId: string, status: string) {
        await this.kafkaService.sendMessage(AppService.kafka_topic_status, { transactionId, status });
    }
    ...
    async receiveResult(message: KafkaMessage) {
        try {
            ...
            console.log("status", this.getStatus(value))
