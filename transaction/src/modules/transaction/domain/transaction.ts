...
export type TransactionProps = ITransactionEssentials &
  Partial<ITransactionOptionals>;
...
export class Transaction extends AggregateRoot {
  ...
  constructor(props: TransactionProps) {
    super();
    if (!validate(props.accountExternalIdCredit))
      throw new BadRequestException(
        DomainMessagesException.ACCOUNT_EXTERNAL_ID_CREDIT,
      );
    if (!validate(props.accountExternalIdDebit))
      throw new BadRequestException(
        DomainMessagesException.ACCOUNT_EXTERNAL_ID_DEBIT,
      );
    if (!validate(props.transactionId))
      throw new BadRequestException(DomainMessagesException.TRANSACTION_ID);
    if (props.value < 0)
      throw new BadRequestException(DomainMessagesException.VALUE_NEGATIVE);
    ...
    this.apply(
      new TransactionCreatedEvent(
        props.transactionId,
        props.status,
        getEnumKeyByEnumValue(ETransferType, props.transferTypeId),
        props.value,
        props.createdAt,
      ),
    );
