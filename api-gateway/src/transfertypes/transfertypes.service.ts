...
export class TransfertypesService {
  ...
  async populateTransfers() {
    ...
    if(totalTransferTypes == 0) {
      TRANSFERTYPES.forEach(transfertype => {
        ...
        this._transferTypeRepository.save(_transfertype)
    })
    }
  }
  

  create(createTransfertypeInput: CreateTransfertypeInput): Promise<Transfertype> {
  ...
  findOne(id: number): Promise<Transfertype> {
