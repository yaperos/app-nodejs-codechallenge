...
// import and use
let event=new TransactionCreatedEvent({id_transaction:'uuidv4',value:100})
// send event
...
let event =new TransactionApprovedEvent({id_transaction:'uuid of transaction'})
// send event
// kafkaproducer.send(event)
````
...
let event =new TransactionRejectedEvent({id_transaction:'uuid of transaction'})
// send event
// kafkaproducer.send(event)
````
